   ...
   while(E_O_F == FALSE)

    {
              #asm("wdr")   
                 ...
                 GetFOTAfile();  //load dual buffers - receiving loop
                ...
                else if(( BUF2_FULL == TRUE)&& (Buf2_Read == FALSE)&& (tmpBuf_full == FALSE))
                {
...
//                              }  
                ...
                {
                   //    _putchar1('M');                   
                       if(rx_counter0 > 0)   //handle partial buffer. last received bytes
                       {
                          BUF1_FULL = TRUE ;
                          Buf1_Read = FALSE;
