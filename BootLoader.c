...
//                   }
         ...
         if((RowsCount < 100) && (Errs == 0))
          twiWriteExtMem1Byte(0,1,2);   //make it eeprom file
          return Errs;
}
...
//       LED1_ON;            //debug    
 ...
 //--------------------------------------------------------
        ...
        //  UBRR1L = baud_57600; 
          ...
          SendPostInfo(2);   //ask server to start FOTA
...
//                       ShowHexString(tmp_buf, BytesCount);    //debug ??????????????? 
                       ...
                       {       
//                              ShowHexString(tmp_buf, 5); 
//                               _putchar1('-');  
                           
                              k = twiWriteExtMemN(0 , Waddress, BytesCount ,tmp_buf);  
                         
                            //   MemoryReadTest(0, 0,5);
                            //    _putchar1('-');  
                       ...
                       {
                              k = twiWriteExtMemN(0, Waddress, PageSize ,tmp_buf);    //67 ticks 
                                                         
//                              if(m == 0)  
//                              {   
//                              
//                                  ShowHexString(tmp_buf, 5);     //debug only
//                                   _putchar1('-');
//                                 
//                              }  
                       }
                      if(k)     //success
                      {
                          ...
                          if(BytesCount > PageSize)      //extra bytes taken to next session
                          {
                              ...
                              tmp_buf[i] = tmp_buf[i + PageSize];  //movee extra bytes to the start of buf
                              gIndx = i;
                          }
...
//            _putchar1('?');
   ...
   void TestMemory(void)
   {
      ...
      //   char buf[] = '0xA5';  
         ...
         k =  twiWriteExtMem1Byte(0, 0x0000,data);
      //   k = twiWriteExtMemN(0, 0xFFF0,1,buf);    //67 ticks 
         if(!k) SendDebugMsg("Memory sec. 1 Write failure..!\r\n\0");       
         else 
         {    
              delay_ms(50);
              ...
              if(i == 0xA5) 
              {
                    SendDebugMsg("Memory sec. 1 Write/Read OK..\r\n\0");  
//                     delay_ms(50);
//                    k =  twiWriteExtMem1Byte(1, 0,data); 
//                    if(!k)
//                      SendDebugMsg("Memory sec. 2 Write failure..!\r\n\0");     
//                    else
//                    {     
//                           delay_ms(50);
//                           i =  twiReadEEP1Byte(1 , 0);  //block 0 
//                           if(i == 0xA5)
//                           SendDebugMsg("Memory sec. 2 Write/Read OK..\r\n\0");
//                           else  SendDebugMsg("Memory sec. 2  failure..!\ Failure..\r\n\0");
//                    }  
                    
              }
                       
              else   SendDebugMsg("Memory Test Failure..\r\n\0");
         }    
