...
{
 //  char BLE_PWR[] = "AT+POWE3\r\n\0";  //6 dbm output power 4 mili watt
 ...
 //  char n[] = "n\0";
    ...
    #asm("sei"); 
       ...
       sprintf(str, "AT+POWE3\r\n\0"); // Set output power to max (6 dbm - 4 mili watt)               
       UART1_WriteMsg(str);            //send string to ble module 
       result = ChkBLERespons("OK","-",25);  //2.5 sec timeout
                                           
       BLE_RESET();                                     
             
      if(UNIT_NAME[3] < 0x38)       //cr16xxxxxxx- cr17xxxxxxx
       UNIT_TYPE = BLE_UNIT;
       else                          //cr18xxxxx
       UNIT_TYPE = MODEM_UNIT;   
       UBRR1L = tBaud;    //restore uart baud 
       if(result)
       {    
          LED1_ON;
          delay_ms(100);                    
...
BYTE GetNextBLETask()
{
 ...
 char *ptr,i,j,k; 
 char STATUS[] = "AT+CIPSTATUS\r\n\0";
 bit  RING_PULSE_PROCESS = FALSE; 
 ...
 //                      SendDebugMsg("\r\nBLE Message..\r\n\0"); 
                     ...
                     if(ptr != NULL)   //CONN  found         
                      ...
                      {    
                           ...
                           if(BLE_While_Modem == TRUE) 
                            toDoList = DO_MODEM; //back to modem task..                                            
                      }      
                  } 
                       ...
                       SendDebugMsg("RX Modem Msg....\r\n\0");
                        ...
                        BLE_RESET_ON;       //Modem session - stop BLE 
                          ...
                          {      
                                ...
                                i = k + 8;
                                while ((RxUart0Buf[k] != '"') && ( k < i)) //look for caller id firat digit
                                k ++; 
                                k ++;                  //k point to first digit of caller num 
                                ...
                                SearchDial = TRUE;     //checked at modem     
                                             //---------------------------Boaz back door----------------------------------------      
                                                 ptr = strstrf(CallerIdBuf,"917237");   //boaz num - eneble back door
                                                 if(ptr != NULL)                     //RING  found         
                                                 {
                         ...
                         {      
                          ...
                          {           
                               ...
                               ptr = strstrf(RxUart0Buf,"DEACT");      //  GPRS diconnection - re connection needed
                               if (ptr != NULL) 
                               {
                                      SendDebugMsg("Connection DEACT - Redial..\r\n\0");                                                                  
                                       SendATCmd(MODEM_SHDN); 
                                      delay_ms(1000); 
                                      FOTA_process = FALSE;
                                      SearchDial = FALSE;
                                      ReDialNeeded = FALSE;;
                                      ATO_needed = FALSE; 
                                      SearchServerDataBase = FALSE;  
                                      RING_PULSE_ACTIVE = FALSE;  
                                      MODEM_GOT_IP = TRUE;
                                      BLECurTask = TASK_BLE_TO_MODEM;  //re connection
                                      return CONTINUE; 
                               }  
                 ...
                 else   if(UNIT_TYPE == MODEM_UNIT)  
                 {
                      ...
                      {      
                             RING_PULSE_ACTIVE = FALSE;  
                             RING_PULSE_PROCESS = TRUE;  
                             delay_ms(500); 
                             SendDebugMsg("RING pulse.\r\n\0"); 
                             
                                                                 
                             BLE_RESET_ON;         //shot ble                             
                             Keep_Alive_timer += 200;  // timer new value to delay keep alive
                             heat_time = 30; // 1 sec 
                              ...
                              {      
                                      ...
                                      {       
                                          ...
                                          Relay_period =  ((RxUart0Buf[index++]-0x30) * 10 + ( RxUart0Buf[index]-0x30));  //in sec
                                           ...
                                           if( Relay_period ==  98) //987 param - stay open
                                           {
                                                if(StayOpen == FALSE)
                                                      StayOpen = TRUE; //dont release relay - leave it open
                                                else
                                                      StayOpen = FALSE;                                                                                                        
...
//                                      } 
                             ...
                             else  //no message yet - get into waiting loop for possible RING
                             {    
                                SendDebugMsg("no message -RING?\r\n\0"); 
                      ...
                      //-------------KEEP ALIVE action------------------------------                                               
                       if(RING_PULSE_PROCESS == FALSE )   //just if ring pulse action ended - avoid colisions
                       {
                              if(KeepAliveMsgNeeded == TRUE) //true if 1 min passed
                              {      
                                                                      
                                     SendSModemString(plusplus,3);      //into command mode
                                    i = ChkNodemRespons("OK","ERR",40);   //wait for response
                                    if(i)
                                    {
                                         UART0_WriteMsg(STATUS);            //check connection status 
                                         i = ChkNodemRespons("T OK","CTED",40);  //expect CONNECT OK or CONNECTED
                                    }  
                                    
                                     if(i) //got it  - else reset unit
                                    {    
                                         if(RING_PULSE_ACTIVE == FALSE ) // if ring not occure while keep alive
                                         {                                                                        
                                            i = Modem_Server_Reconnect();  //re connect and send keep alive
                                              if(i == 1)
                                             {    
                                         ...
                                         else ATO_needed = TRUE;   //else dont re connect and wait for ring string
                                         return WAIT; 
                                    } 
                                     
                                    SendATCmd(MODEM_SHDN);  //connection failed - shut modem and reset unit
                                    delay_ms(500);
                                    #asm("jmp 0");                                 
                              }
                        }                                        
          ...
          break;   
           ...
           case TASK_BLE_TO_MODEM:  
               ...
               {  
                   ...
                   Init_Modem_Vars(); // go to modem  task 
//                   BLE_RESET_OFF; 
               }             
